...
(define (eval-1 exp)
  (cond ((constant? exp) exp)
	...
	((quote-exp? exp) (cadr exp))
        ((and-exp? exp) (and-exp (cdr exp)))
	((if-exp? exp)
	 (if (eval-1 (cadr exp))
	     (eval-1 (caddr exp))
	     (eval-1 (cadddr exp))))
        ((let-exp? exp) (apply-let exp))
	((lambda-exp? exp) exp)
...
(define (apply-let exp)
  (eval-1 (substitute (caddr exp)   ; the body
			     (extract-let-formal-params (cadr exp))    ; the formal parameters
			     (eval-let-params (extract-let-args (cadr exp)))         ; the actual arguments
			     '())))
  ...
  (cond ((constant? exp) exp)
	...
	((lambda-exp? exp)
	 (list 'lambda
        ...
        ((let-exp? exp)
	 (list 'let
	       (substitute (cadr exp) params args (append bound (extract-let-formal-params (cadr exp))))
	       (substitute (caddr exp) params args (append bound (extract-let-formal-params (cadr exp))))))
...
((lambda (pred?  seq)
     ((lambda (filter) (filter filter pred? seq))
      (lambda (filter pred? seq)
        (cond ((null? seq) null)
              ((pred? (car seq)) (cons (car seq) (filter filter pred? (cdr seq))))
              (else (filter filter pred? (cdr seq)))))))
     number?
     '(1 car 2 5 rain))
...
(define (and-exp args)
  (cond ((null? args) #t)
         ((not (eval-1 (car args))) #f)
         (else (and-exp (cdr args)))))
...
(define (extract-let-formal-params args)
    (map1 first args))

(define (extract-let-args args)
    (map1 cadr args))

(define (eval-let-params params)
    (map (lambda (element) (eval-1 element)) params))
