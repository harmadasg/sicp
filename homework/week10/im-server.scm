	  ...
	  (begin	    
		...
		(if port-to-dst		   
	     ...
	     ((equal? 'broadcast (request-action req))
	     	(broadcast (request-data req)))
	     	
	     ((equal? 'blocked (request-action req))
		(send-request (make-request 'server 
	       		(request-data req)
		        'receive-msg
		        '(Cannot send message (blocked)))
		 (find-port-to-client (request-data req))))				  			  
...
(define (server-broadcast cmd data)
 ...
 (define (broadcast message)
  (format logging "Broadcasting message to all clients~%~%")
  (for-each (lambda (name)
	      (send-request (make-request 'server name 'receive-msg message)
			    (find-port-to-client name)))
	    (get-clients-list))
  (format logging "Broadcast done.~%~%")) 
