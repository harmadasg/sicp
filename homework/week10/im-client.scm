...
(define blocked-users '())  	; List of blocked users
  ...
  (define (request-handler)
    (let ((req (get-request port-from-server)))
	  ...
	  (begin
	    ...
	    (cond
	     ((memq (request-src req) blocked-users)
	      (send-request (make-request whoiam 'server 'blocked (request-src req)) port-to-server))
...
(define (im-group users message)
  (for-each (lambda (user) (im user message)) users))
...
(define (block-user user)
  (set! blocked-users (cons user blocked-users)))
